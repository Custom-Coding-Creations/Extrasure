...
export default async function Home() {
  ...
  return (
    <div>
      <section className="relative overflow-hidden border-b" style={{ borderColor: "rgba(242, 236, 228, 0.24)", background: "linear-gradient(145deg, #201a18 0%, #2b2421 55%, #171211 100%)", color: "#f3ebe2" }}>
        <div className="absolute inset-0 bg-[radial-gradient(circle_at_top_right,_rgba(142,47,61,0.24),_transparent_52%)]" />
        <div className="relative mx-auto grid w-full max-w-6xl gap-8 px-4 py-14 sm:px-6 lg:grid-cols-2 lg:px-8 lg:py-20">
          <div>
            ...
            <div className="mt-7 flex flex-wrap gap-3">
              ...
              <Link href="/contact" className="rounded-full border px-5 py-3 text-sm font-semibold transition hover:bg-white hover:text-[#1f1a18]" style={{ borderColor: "rgba(243, 235, 226, 0.54)", color: "#f3ebe2" }}>
      ...
      <section className="mx-auto w-full max-w-6xl px-4 py-14 sm:px-6 lg:px-8">
        <div className="mb-6 flex items-end justify-between gap-6">
          ...
          <Link href="/services" className="text-sm font-semibold underline underline-offset-4" style={{ color: "#1f1a18", textDecorationColor: "#8e2f3d" }}>
            View all services
          </Link>
        </div>
        <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-4">
          {services.slice(0, 8).map((service) => (
            <article key={service.slug} className="paper-panel rounded-2xl border p-5" style={{ borderColor: "rgba(47, 36, 29, 0.17)" }}>
              ...
              <Link href={`/services/${service.slug}`} className="mt-4 inline-block text-sm font-semibold underline underline-offset-4" style={{ color: "#1f1a18", textDecorationColor: "#8e2f3d" }}>
                Learn more
              </Link>
            </article>
          ))}
