...
export default async function Home() {
  ...
  return (
    <div>
      <section className="relative overflow-hidden border-b" style={{ borderColor: "rgba(30, 33, 71, 0.08)", background: "linear-gradient(145deg, #1e2147 0%, #2d3170 55%, #ff6f61 140%)", color: "#fff6ef" }}>
        <div className="absolute inset-0 bg-[radial-gradient(circle_at_top_right,_rgba(255,228,212,0.28),_transparent_52%)]" />
        <div className="relative mx-auto grid w-full max-w-6xl gap-8 px-4 py-14 sm:px-6 lg:grid-cols-2 lg:px-8 lg:py-20">
          <div>
            ...
            <h1 className="mt-4 text-4xl leading-tight sm:text-5xl" style={{ color: "#fff6ef" }}>
              Pest Problems Solved Fast, with Service You Can Trust
            </h1>
            <p className="mt-5 max-w-xl text-base leading-7" style={{ color: "rgba(255, 241, 231, 0.82)" }}>
            ...
            <div className="mt-7 flex flex-wrap gap-3">
              ...
              <Link href="/contact" className="rounded-full border px-5 py-3 text-sm font-semibold transition hover:bg-white hover:text-[#1e2147]" style={{ borderColor: "rgba(255,241,231,0.4)", color: "#fff6ef", background: "rgba(255,255,255,0.06)" }}>
                {company.ctaSecondary}
              </Link>
            </div>
            <ul className="mt-6 flex flex-wrap gap-2 text-xs" style={{ color: "rgba(255,241,231,0.76)" }}>
              {trustBadges.map((badge) => (
                <li key={badge} className="rounded-full border px-3 py-1" style={{ borderColor: "rgba(255,241,231,0.22)", background: "rgba(255,255,255,0.07)" }}>
                  {badge}
                </li>
              ))}
            </ul>
