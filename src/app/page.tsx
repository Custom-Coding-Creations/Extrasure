...
export default async function Home() {
  ...
  return (
    <div>
      <section className="relative overflow-hidden border-b" style={{ borderColor: "rgba(20, 33, 46, 0.08)", background: "linear-gradient(145deg, #14212e 0%, #1d3144 55%, #0f1a24 100%)", color: "#edf3f8" }}>
        <div className="absolute inset-0 bg-[radial-gradient(circle_at_top_right,_rgba(202,169,106,0.2),_transparent_52%)]" />
        <div className="relative mx-auto grid w-full max-w-6xl gap-8 px-4 py-14 sm:px-6 lg:grid-cols-2 lg:px-8 lg:py-20">
          <div>
            ...
            <p className="mt-5 max-w-xl text-base leading-7" style={{ color: "rgba(237, 243, 248, 0.82)" }}>
            ...
            <div className="mt-7 flex flex-wrap gap-3">
              ...
              <Link href="/contact" className="rounded-full border px-5 py-3 text-sm font-semibold transition hover:bg-white hover:text-[#14212e]" style={{ borderColor: "rgba(237,243,248,0.34)", color: "#edf3f8" }}>
                {company.ctaSecondary}
              </Link>
            </div>
            <ul className="mt-6 flex flex-wrap gap-2 text-xs" style={{ color: "rgba(237,243,248,0.72)" }}>
              {trustBadges.map((badge) => (
                <li key={badge} className="rounded-full border px-3 py-1" style={{ borderColor: "rgba(237,243,248,0.24)", background: "rgba(255,255,255,0.05)" }}>
                  {badge}
                </li>
              ))}
            </ul>
