...
export default async function Home() {
  ...
  return (
    <div>
      <section className="relative overflow-hidden border-b" style={{ borderColor: "rgba(73, 102, 255, 0.12)", background: "linear-gradient(135deg, #121a3d 0%, #2c3b9b 42%, #6f7cff 70%, #00c2ff 100%)", color: "#edf2ff" }}>
        <div className="absolute inset-0 bg-[radial-gradient(circle_at_top_right,_rgba(255,255,255,0.24),_transparent_52%)]" />
        <div className="relative mx-auto grid w-full max-w-6xl gap-8 px-4 py-14 sm:px-6 lg:grid-cols-2 lg:px-8 lg:py-20">
          <div>
            ...
            <h1 className="mt-4 text-4xl leading-tight sm:text-5xl" style={{ color: "#ffffff" }}>
              Pest Problems Solved Fast, with Service You Can Trust
            </h1>
            <p className="mt-5 max-w-xl text-base leading-7" style={{ color: "rgba(237, 242, 255, 0.82)" }}>
            ...
            <div className="mt-7 flex flex-wrap gap-3">
              ...
              <Link href="/contact" className="rounded-full border px-5 py-3 text-sm font-semibold transition hover:bg-white hover:text-[#121a3d]" style={{ borderColor: "rgba(237,242,255,0.3)", color: "#edf2ff", background: "rgba(255,255,255,0.08)" }}>
                {company.ctaSecondary}
              </Link>
            </div>
            <ul className="mt-6 flex flex-wrap gap-2 text-xs" style={{ color: "rgba(237,242,255,0.76)" }}>
              {trustBadges.map((badge) => (
                <li key={badge} className="rounded-full border px-3 py-1" style={{ borderColor: "rgba(237,242,255,0.2)", background: "rgba(255,255,255,0.08)" }}>
                  {badge}
                </li>
              ))}
            </ul>
