...
export async function AdminShell({ title, subtitle, children }: AdminShellProps) {
  ...
  return (
    <div className="mx-auto w-full max-w-7xl px-4 py-8 sm:px-6 lg:px-8">
      ...
      <div className="mb-6 rounded-2xl border p-5" style={{ borderColor: "rgba(20,33,46,0.08)", background: "linear-gradient(150deg, #ffffff 0%, #eef3f7 100%)", boxShadow: "var(--shadow-md)" }}>
        <div className="flex flex-wrap items-center justify-between gap-3">
          <p className="text-xs uppercase tracking-[0.18em]" style={{ color: "rgba(20,33,46,0.56)" }}>Owner Operations Console</p>
          <div className="flex items-center gap-2">
            <p className="rounded-full px-3 py-1 text-xs" style={{ background: "#eef3f7", color: "#22364a" }}>
        ...
        <h1 className="mt-1 text-3xl" style={{ color: "#14212e" }}>{title}</h1>
        <p className="mt-2 max-w-3xl text-sm" style={{ color: "rgba(20,33,46,0.72)" }}>{subtitle}</p>
      </div>
      <div className="grid gap-6 lg:grid-cols-[220px_1fr]">
        <aside className="rounded-2xl border p-4" style={{ borderColor: "rgba(20,33,46,0.08)", background: "#ffffff", boxShadow: "var(--shadow-sm)" }}>
          <AdminNav />
        </aside>
