...
export function AccountShell({ title, subtitle, activePath, children, logoutAction, shellQuickActions, shellNotifications }: AccountShellProps) {
  ...
  return (
    <div className="dashboard-shell mx-auto w-full max-w-7xl px-4 pb-28 pt-8 sm:px-6 lg:px-8 lg:pb-10 lg:pt-12">
      <div className="account-hero-layer relative z-10 overflow-hidden rounded-3xl border p-5 backdrop-blur-sm sm:p-7" style={{ borderColor: "rgba(18, 99, 156, 0.18)", background: "rgba(255, 255, 255, 0.82)" }}>
        <div aria-hidden className="pointer-events-none absolute inset-x-0 top-0 h-28 bg-[radial-gradient(circle_at_12%_18%,rgba(14,165,233,0.2),transparent_56%),radial-gradient(circle_at_88%_6%,rgba(67,124,255,0.16),transparent_48%)]" />
        <div className="relative flex flex-wrap items-start justify-between gap-4">
        <div className="max-w-3xl">
          <p className="text-xs uppercase tracking-[0.2em]" style={{ color: "rgba(18, 99, 156, 0.62)" }}>ExtraSure Home Platform</p>
          <h1 className="mt-2 text-3xl sm:text-4xl" style={{ color: "#163b67" }}>{title}</h1>
          <p className="mt-3 text-sm sm:text-base" style={{ color: "rgba(19, 33, 54, 0.72)" }}>{subtitle}</p>
          <div className="mt-4 flex flex-wrap gap-2">
            <span className="rounded-full border px-3 py-1 text-[0.68rem] font-semibold uppercase tracking-[0.12em]" style={{ borderColor: "rgba(18, 99, 156, 0.22)", background: "#f5faff", color: "rgba(19, 33, 54, 0.72)" }}>
        ...
        <div className="flex flex-wrap items-center justify-end gap-2">
          ...
          <form action={logoutAction}>
            <button
              type="submit"
                className="elevated-action rounded-full border bg-white px-5 py-2 text-sm font-semibold transition hover:bg-[#124482] hover:text-white"
                style={{ borderColor: "#124482", color: "#124482" }}
            >
              Sign Out
            </button>
          </form>
          ...
          return (
            <Link
              ...
              className={`rounded-full px-4 py-2 text-sm font-semibold transition focus-visible:focus-ring ${
                active
                  ? "text-white"
                  : "border bg-[#ffffff] hover:bg-[#edf6ff]"
              }`}
              style={active ? { background: "#124482" } : { borderColor: "rgba(18, 99, 156, 0.2)", color: "#124482" }}
            >
              {link.label}
            </Link>
          );
      ...
      {mobileQuickActions.length ? (
        <section className="relative z-10 mt-4 lg:hidden" aria-label="Mobile smart shortcuts">
          <div className="mb-2 flex items-center justify-between gap-3 px-1">
            <p className="text-[0.68rem] font-semibold uppercase tracking-[0.16em]" style={{ color: "rgba(18, 99, 156, 0.58)" }}>Smart shortcuts</p>
            <p className="text-[0.68rem] font-semibold uppercase tracking-[0.14em]" style={{ color: "rgba(18, 99, 156, 0.52)" }}>Thumb-ready actions</p>
          </div>
          <div className="-mx-1 flex snap-x gap-3 overflow-x-auto px-1 pb-1 [scrollbar-width:none] [&::-webkit-scrollbar]:hidden">
            {mobileQuickActions.map((action) => (
              ...
              >
                <p className="text-[0.68rem] font-semibold uppercase tracking-[0.15em]" style={{ color: "rgba(18, 99, 156, 0.58)" }}>Quick action</p>
                <p className="mt-2 text-sm font-semibold" style={{ color: "#163b67" }}>{action.label}</p>
              </Link>
            ))}
          </div>
        </section>
        ...
        {accountLinks.slice(0, 5).map((link) => {
          ...
          return (
            <Link
              ...
              style={active ? { background: "#124482" } : { color: "#124482" }}
            >
              {link.shortLabel}
            </Link>
          );
